#let _the-thm-counter-at(place) = (
  numbering("1.1.", ..counter(heading).at(place)) + numbering("1", counter("thm").at(place).first() + 1)
)
    ...
    block(
      ...
      inset: (left: 1em, y: 5pt),
      {
        block({
          ...
          theorem-type
          h(0.5em)
          context counter(heading).display("1.1.") + counter(figure).display("1")
          h(2em)
          alias
        })
        content
